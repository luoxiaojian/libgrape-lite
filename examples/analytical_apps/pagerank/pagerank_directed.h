...
#include "pagerank/pagerank_directed_context.h"
...
class PageRankDirected : public BatchShuffleAppBase<FRAG_T, PageRankDirectedContext<FRAG_T>>,
                 public ParallelEngine,
                 public Communicator {
 public:
  INSTALL_BATCH_SHUFFLE_WORKER(PageRankDirected<FRAG_T>, PageRankDirectedContext<FRAG_T>,
                               FRAG_T)
  ...
  void IncEval(const fragment_t& frag, context_t& ctx,
    ...
    if (ctx.avg_degree > 10 && frag.fnum() > 1) {
...
#endif
      ForEach(
          inner_vertices, [src_fid, &frag, &ctx, base, &dangling_sums](int tid, vertex_t u) {
            double cur = ctx.next_result[u];
            auto es = frag.GetIncomingAdjList(u, src_fid);
            for (auto& e : es) {
              cur += ctx.result[e.get_neighbor()];
      ...
      ctx.exec_time -= GetCurrentTime();
#endif
      ForEach(inner_vertices, [&ctx, &frag, base, &dangling_sums](int tid, vertex_t u) {
        double cur = 0;
