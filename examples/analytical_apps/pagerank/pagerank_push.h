...
class PageRankPush : public ParallelAppBase<FRAG_T, PageRankPushContext<FRAG_T>>, public ParallelEngine, public Communicator {
 public:
  INSTALL_PARALLEL_WORKER(PageRankPush<FRAG_T>, PageRankPushContext<FRAG_T>, FRAG_T)
  ...
  void PEval(const fragment_t& frag, context_t& ctx, message_manager_t& messages) {
    ...
    ForEach(outer_vertices, [&ctx, &messages, &frag](int tid, vertex_t u) {
      messages.SyncStateOnOuterVertex<fragment_t, double>(frag, u, ctx.next_result[u], tid);
      ctx.next_result[u] = 0;
    });
  ...
  void IncEval(const fragment_t& frag, context_t& ctx, message_manager_t& messages) {
    ...
    messages.ParallelProcess<fragment_t, double>(thread_num(), frag,
                                                 [&ctx](int tid, vertex_t u, double msg) {
