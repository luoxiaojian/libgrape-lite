...
#if 0
  timer_next("run algorithm");
  using context_t = typename APP1_T::worker_t::context_t;
  std::shared_ptr<context_t> common_context;
  auto worker1 = APP1_T::CreateWorker(app1, fragment);
  worker1->Init(comm_spec, spec);
  worker1->Query(std::forward<Args>(args)...);
  common_context = worker1->GetContext();
  worker1->Finalize();
  worker1.reset();
  auto worker2 = std::make_shared<typename APP2_T::worker_t>(app2, fragment,
                                                             common_context);
...
void Run() {
  ...
  if (name.find("sssp") != std::string::npos) {
    ...
    if (name == "bfs_auto") {
      ...
      //   CreateAndQuery<OID_T, VID_T, VDATA_T, EmptyType,
      //   LoadStrategy::kOnlyOut,
      //                  PageRankLocal, double, int>(comm_spec, out_prefix,
      //                  fnum,
      //                                              spec, FLAGS_pr_d,
      //                                              FLAGS_pr_mr);
      ...
      //   CreateAndQuery<OID_T, VID_T, VDATA_T, EmptyType,
      //   LoadStrategy::kBothOutIn,
      //                  PageRankAuto, double, int>(comm_spec, out_prefix,
      //                  fnum,
      //                                             spec, FLAGS_pr_d,
      //                                             FLAGS_pr_mr);
      ...
      //   CreateAndQuery<OID_T, VID_T, VDATA_T, EmptyType,
      //   LoadStrategy::kOnlyOut,
      //                  PageRank, double, int>(comm_spec, out_prefix, fnum,
      //                  spec,
      //                                         FLAGS_pr_d, FLAGS_pr_mr);
      // } else if (name == "pagerank_push") {
      //   CreateAndQuery<OID_T, VID_T, VDATA_T, EmptyType,
      //   LoadStrategy::kOnlyOut,
      //                  PageRankPush, double, int>(comm_spec, out_prefix,
      //                  fnum,
      //                                             spec, FLAGS_pr_d,
      //                                             FLAGS_pr_mr);
      ...
      // CreateAndQuery<OID_T, VID_T, VDATA_T, EmptyType,
      // LoadStrategy::kOnlyOut,
      //                BC, OID_T>(comm_spec, out_prefix, fnum, spec,
      //                           FLAGS_bc_source);
