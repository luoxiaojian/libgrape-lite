...
void RunUndirectedPageRankOpt(const CommSpec& comm_spec,
  ...
  graph_spec.set_load_strategy(LoadStrategy::kOnlyOut);
  graph_spec.set_partition_strategy(FLAGS_segmented_partition
                                        ? PartitionStrategy::kSegmented
                                        : PartitionStrategy::kHash);
  using VertexMapType = GlobalVertexMap<int64_t, uint32_t>;
...
void RunOpt() {
  ...
  if (name == "sssp") {
    ...
    if (FLAGS_directed) {
      ...
      RunUndirectedPageRankOpt(comm_spec, out_prefix, spec, FLAGS_pr_d,
                               FLAGS_pr_mr);
    }
  } else if (name == "cdlp") {
    if (FLAGS_directed) {
