...
#ifndef EXAMPLES_ANALYTICAL_APPS_LCC_LCC_SORT_BETA_H_
#define EXAMPLES_ANALYTICAL_APPS_LCC_LCC_SORT_BETA_H_
...
class LCCSortBeta : public ParallelAppBase<FRAG_T, LCCBetaContext<FRAG_T, COUNT_T>>,
 ...
 public:
  ...
  using worker_t = ParallelWorker<LCCSortBeta<FRAG_T, COUNT_T>>;
  ...
  static std::shared_ptr<worker_t> CreateWorker(
      std::shared_ptr<LCCSortBeta<FRAG_T, COUNT_T>> app,
  ...
  void IncEval(const fragment_t& frag, context_t& ctx,
    ...
    if (ctx.stage == 0) {
      ...
      if (ctx.degree_threshold == std::numeric_limits<int>::max()) {
        ...
        ForEach(inner_vertices, [&frag, &ctx, &messages, &max_degrees](int tid, vertex_t v) {
	  ...
	  if (msg_vec.size() > max_degrees[tid]) {
        ...
        ForEach(inner_vertices, [this, &frag, &ctx, &messages, &max_degrees](int tid,
                                                               vertex_t v) {
      ...
      ctx.exec_time -= GetCurrentTime();
#endif
      std::vector<DenseVertexSet<typename FRAG_T::inner_vertices_t>> inner_vertexsets(
          thread_num());
      std::vector<DenseVertexSet<typename FRAG_T::outer_vertices_t>> outer_vertexsets(thread_num());
      ...
      if (ctx.degree_threshold == std::numeric_limits<int>::max()) {
        ForEach(inner_vertices, [this, &ctx, &inner_vertexsets, &outer_vertexsets](int tid, vertex_t v) {
          auto& v0_inner_nbr_vec = ctx.complete_inner_neighbor[v];
          auto& v0_outer_nbr_vec = ctx.complete_outer_neighbor[v];
	  if (v0_inner_nbr_vec.size() + v0_outer_nbr_vec.size() <= 1) {
	    return ;
	  }
#if 0
          count_t v_count = 0;
          for (auto u : v0_inner_nbr_vec) {
            ...
            count_t u_count = intersect(v0_inner_nbr_vec, v1_inner_nbr_vec, ctx.tricnt);
	    u_count += intersect(v0_outer_nbr_vec, v1_outer_nbr_vec, ctx.tricnt);
	  ...
	  auto& v0_outer_nbr_set = outer_vertexsets[tid];
        ...
        ForEach(inner_vertices, [this, &frag, &ctx, &inner_vertexsets, &outer_vertexsets](int tid, vertex_t v) {
          if (filterByDegree(frag, ctx, v)) {
            return;
          }
          auto& v0_inner_nbr_vec = ctx.complete_inner_neighbor[v];
...
#endif  // EXAMPLES_ANALYTICAL_APPS_LCC_LCC_SORT_BETA_H_
