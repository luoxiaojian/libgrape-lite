...
#ifndef EXAMPLES_ANALYTICAL_APPS_LCC_LCC_OPT_L_H_
#define EXAMPLES_ANALYTICAL_APPS_LCC_LCC_OPT_L_H_
...
class LCCOptL : public ParallelAppBase<FRAG_T, LCCOptContext<FRAG_T, COUNT_T>,
 ...
 public:
  ...
  using worker_t = ParallelWorkerOpt<LCCOptL<FRAG_T, COUNT_T>>;
  ...
  static std::shared_ptr<worker_t> CreateWorker(
      std::shared_ptr<LCCOptL<FRAG_T, COUNT_T>> app,
...
class LCCOptL<FRAG_T, COUNT_T,
 ...
 public:
  ...
  void PEval(const fragment_t& frag, context_t& ctx,
    ...
    LOG(INFO) << "Calculating intersection with: " << ctx.intersection_name;
    
    intersect_func = SIMDCompressionLib::IntersectionFactory::getFromName(ctx.intersection_name);
  ...
  count_t intersect(lcc_opt_impl::ref_vector<vertex_t>& lhs,
                    ...
                    tricnt_list_t& result) {
    uint32_t* set_a = reinterpret_cast<uint32_t*>(lhs.data());
    size_t size_a = lhs.size();
    uint32_t* set_b = reinterpret_cast<uint32_t*>(rhs.data());
    size_t size_b = rhs.size();

    static thread_local std::vector<vertex_t> vec_c;
    vec_c.clear();
    vec_c.resize(std::min(size_a, size_b));
    uint32_t* set_c = reinterpret_cast<uint32_t*>(vec_c.data());

    size_t size_c = intersect_func(set_a, size_a, set_b, size_b, set_c);
    vec_c.resize(size_c);
  ...
  SIMDCompressionLib::intersectionfunction intersect_func;
};
...
#endif  // EXAMPLES_ANALYTICAL_APPS_LCC_LCC_OPT_L_H_
