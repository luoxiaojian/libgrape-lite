#ifndef GRAPE_FRAGMENT_EV_FRAGMENT_LOADER_BETA_H_
#define GRAPE_FRAGMENT_EV_FRAGMENT_LOADER_BETA_H_
...
class EVFragmentLoaderBeta {
 ...
 public:
  explicit EVFragmentLoaderBeta(const CommSpec& comm_spec)
      : comm_spec_(comm_spec) {}

  ~EVFragmentLoaderBeta() = default;
  ...
  std::shared_ptr<fragment_t> LoadFragment(const std::string& efile,
    ...
    std::shared_ptr<vertex_map_t> vm_ptr = std::make_shared<vertex_map_t>(comm_spec_);
    ...
    for (fid_t i = 0; i < fnum; ++i) {
      CHECK_LT(degree_lists[i].size(), static_cast<size_t>(std::numeric_limits<int>::max()));
      MPI_Allreduce(MPI_IN_PLACE, degree_lists[i].data(), degree_lists[i].size(), MPI_INT, MPI_SUM, comm_spec_.comm());
    }
    ...
    size_t total_score = total_edge_num + total_vertex_num * spec.rebalance_vertex_factor;
    ...
    if (comm_spec_.worker_id() == 0) {
      ...
      for (fid_t i = 0; i < fnum; ++i) {
        LOG(INFO) << "[frag-" << i << "]: vertex_num: " << degree_lists[i].size() << " -> " << vnum_list[i] << ", edge_num: " << enum_before[i] << " -> " << enum_after[i] << ", score: " << scores_before[i] << " ->" << scores_after[i];
      }
    }
    ...
    for (fid_t i = 0; i < fnum; ++i) {
      ...
      edges_to_frag[i].Init(comm_spec_.comm(), edge_tag);
	...
	auto& buffers = data_in.buffers();
        foreach_rval(buffers, [&](vid_t&& src, vid_t&& dst,
                                     edata_t&& data) {
...
#endif  // GRAPE_FRAGMENT_EV_FRAGMENT_LOADER_BETA_H_
