...

    {
      ...
      if (load_strategy == LoadStrategy::kOnlyOut) {
        ...
        for (size_t i = 0; i < evec_size; ++i) {
          ...
          e.set_src(gid_maps_[old_src_fid][vm_ptr->GetLidFromGid(old_e.src())]);
          e.set_dst(gid_maps_[old_dst_fid][vm_ptr->GetLidFromGid(old_e.dst())]);
    ...
    std::thread deltaEdgesRecvThread([&]() {
      ...
      while (!data_in.Finished()) {
        ...
        while (src_iter != src_end) {
          buf_ptr->set_src(*src_iter++);
          buf_ptr->set_dst(*dst_iter++);
