...
class MutableEdgecutFragment
 ...
 public:
  ...
  template <typename T>
  using outer_vertex_array_t = VertexArrayBeta<outer_vertices_t, T>;

  template <typename T>
  using vertex_array_t = VertexArrayBeta<vertices_t, T>;
  ...
  void AddEdges(std::vector<edge_t>& edges) {
    ...
    this->outer_vertices_.SetRange(id_parser_.max_local_id() - ovnum_,
  id_parser_.max_local_id()); this->vertices_.SetRange(0, ivnum_,
  id_parser_.max_local_id() - ovnum_, id_parser_.max_local_id());

    oe_.add_edges(edges);
    ie_.add_reversed_edges(edges);
  }
   */
  ...
  inline const sub_vertices_t& OuterVertices(fid_t fid) const override {
 ...
 private:
  ...
  VertexArrayBeta<inner_vertices_t, nbr_t*> iespliter_, oespliter_;
